...
export const NavItem: React.FC<NavItemProps> = ({ icon, label, active, className = '', ...props }) => {
  return (
    <button
...
export const Sidebar: React.FC<SidebarProps> = ({ children, defaultCollapsed = false, className = '', ...props }) => {
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between p-4 border-b border-gray-100 h-16">
        {!collapsed && <span className="font-bold text-[#4A0E8F] text-lg truncate">CyberVast LMS</span>}
        <button
      ...
      <div className={`flex-1 overflow-y-auto p-4 flex flex-col gap-2 ${collapsed ? 'items-center' : ''}`}>
        {children}
      </div>
    </aside>
  );
